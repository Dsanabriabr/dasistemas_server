...
var relationFieldOnNodeType = exports.relationFieldOnNodeType = function relationFieldOnNodeType(_ref2) {
  ...
  var filterPredicates = buildFilterPredicates(fieldArgs, innerSchemaType, nestedVariable, resolveInfo, selectionFilters, paramIndex);
...
var nodeQuery = function nodeQuery(_ref11) {
  ...
  if (filterParam) params['filter'] = transformExistentialFilterParams(filterParam);
  ...
  var filterPredicates = buildFilterPredicates(fieldArgs, schemaType, variableName, resolveInfo, nonNullParams, rootParamIndex);
  ...
  var query = 'MATCH (' + safeVariableName + ':' + safeLabelName + (argString ? ' ' + argString : '') + ') ' + predicate + ('RETURN ' + safeVariableName + ' {' + subQuery + '} AS ' + safeVariableName + orderByValue + outerSkipLimit);
...
var buildFilterPredicates = function buildFilterPredicates(fieldArgs, schemaType, variableName, resolveInfo, params, paramIndex) {
  ...
  if (filterArg && filterValue) {
    ...
    var filterParam = paramIndex > 1 ? '$' + (paramIndex - 1) + '_filter' : '$filter';
    // recursively translate argument filterParam relative to schemaType
    filterPredicates = translateFilterArguments(schemaType, variableName, typeFields, filterParam, schema, filterValue);
  }
  return filterPredicates;
};

var translateFilterArguments = function translateFilterArguments(schemaType, variableName, typeFields, filterParam, schema, filterValue, parentVariableName) {
  ...
  return (0, _entries2.default)(filterValue).reduce(function (predicates, _ref19) {
...
var translateFilterArgument = function translateFilterArgument(_ref21) {
  ...
  var parameterPath = (parentVariableName ? parentVariableName : filterParam) + '.' + fieldName;
...
var translateScalarFilter = function translateScalarFilter(_ref22) {
  ...
  return '' + nullFieldPredicate + buildScalarFilterPredicate(filterOperationType, propertyPath) + ' ' + parameterPath;
};
...
var decideNullSkippingPredicate = function decideNullSkippingPredicate(_ref23) {
  ...
  return isListFilterArgument && parentVariableName ? parameterPath + ' IS NULL OR ' : '';
};
...
var translateNullFilter = function translateNullFilter(_ref24) {
  ...
  var paramPath = (parentVariableName ? parentVariableName : filterParam) + '._' + filterOperationField + '_' + (isNegationFilter ? 'not_' : '') + 'null';
...
var buildScalarFilterPredicate = function buildScalarFilterPredicate(filterOperationType, propertyPath) {
...
var translateRelationshipFilter = function translateRelationshipFilter(_ref27) {
  ...
  var innerSchemaType = (0, _utils.innerType)(schemaType.getFields()[filterOperationField].type);
  ...
  var pathExistencePredicate = buildRelationshipExistencePath(variableName, relLabel, relDirection, relatedTypeName);
  if (isExistentialFilter(filterOperationType, filterValue)) {
    return translateNullFilter({
  ...
  var safeRelVariableName = (0, _utils.safeVar)(schemaTypeNameLow + '_filter_' + relatedTypeNameLow);
...
var decidePredicateFunction = function decidePredicateFunction(_ref29) {
  ...
  if (filterOperationField === 'AND') return 'ALL';else if (filterOperationField === 'OR') return 'ANY';else if (isRelation) {
    switch (filterOperationType) {
      case 'not':
        return 'NONE';
      case 'in':
...
var buildRelationshipListPattern = function buildRelationshipListPattern(_ref30) {
  ...
  return '[(' + safeFromVar + ')' + (relDirection === 'IN' ? '<' : '') + '-[' + relVar + ':' + relLabel + ']-' + (relDirection === 'OUT' ? '>' : '') + '(' + safeToVar + ':' + toLabel + ') | ' + safeToVar + ']';
};

var buildRelationshipExistencePath = function buildRelationshipExistencePath(fromVar, relLabel, relDirection, toType) {
  var safeFromVar = (0, _utils.safeVar)(fromVar);
  return '(' + safeFromVar + ')' + (relDirection === 'IN' ? '<' : '') + '-[:' + relLabel + ']-' + (relDirection === 'OUT' ? '>' : '') + '(:' + toType + ')';
};
...
var buildUniquePredicates = function buildUniquePredicates(_ref31) {
  var schemaType = _ref31.schemaType,
      ...
      isListFilterArgument = _ref31$isListFilterAr === undefined ? false : _ref31$isListFilterAr;
  ...
  return filterValue.reduce(function (predicates, filter) {
...
var transformExistentialFilterParams = exports.transformExistentialFilterParams = function transformExistentialFilterParams(filterParam) {
