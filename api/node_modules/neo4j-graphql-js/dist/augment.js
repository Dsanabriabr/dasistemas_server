    ...
    queryNames.forEach(function (t) {
      ...
      if ((0, _utils.isNodeType)(valueType) && shouldAugmentType(config, 'query', valueTypeName)) {
        ...
        if (!(0, _utils.getFieldDirective)(field, 'cypher')) {
          queryMap[t].arguments = possiblyAddArgument(args, 'filter', '_' + valueTypeName + 'Filter');
        }
      }
    });
...
var possiblyAddFilterInput = function possiblyAddFilterInput(astNode, typeMap, resolvers, config) {
  ...
  if ((0, _utils.isNodeType)(astNode) && shouldAugmentType(config, 'query', typeName)) {
    ...
    var filterFields = buildFilterFields(name, astNode, typeMap, resolvers, config);
    if (typeMap[name] === undefined && filterFields.length) {
      typeMap[name] = (0, _graphql.parse)('input ' + name + ' {' + filterFields.join('') + '}');
    }
...
var buildFilterFields = function buildFilterFields(filterType, astNode, typeMap, resolvers, config) {
  var fields = astNode.fields;
    ...
    if (fieldIsNotIgnored(astNode, t, resolvers) && isNotSystemField(fieldName) && !(0, _utils.getFieldDirective)(t, 'cypher')) {
      ...
      if (!isList) {
        if (valueTypeName === 'ID' || valueTypeName == 'String') {
          ...
          filters.push('\n            ' + fieldName + ': ' + valueTypeName + '\n            ' + fieldName + '_not: ' + valueTypeName + '\n            ' + fieldName + '_in: [' + valueTypeName + '!]\n            ' + fieldName + '_not_in: [' + valueTypeName + '!]\n          ');
        } else if ((0, _utils.isKind)(valueType, 'ObjectTypeDefinition') && (0, _utils.getFieldDirective)(t, 'relation') && shouldAugmentType(config, 'query', valueTypeName)) {
...
var possiblyAddTypeFieldArguments = function possiblyAddTypeFieldArguments(astNode, typeMap, resolvers, config, queryType) {
  ...
  fields.forEach(function (field) {
    ...
    (0, _utils.isNodeType)(relationType) && ((0, _utils.getFieldDirective)(field, 'relation') || (0, _utils.getFieldDirective)(field, 'cypher'))) {
      args = field.arguments;
      if ((0, _utils._isListType)(field)) {
      ...
      if (!(0, _utils.getFieldDirective)(field, 'cypher')) {
        field.arguments = possiblyAddArgument(args, 'filter', '_' + relationTypeName + 'Filter');
      }
    }
  });
  return fields;
